    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register([Bind(Exclude = "IsEmailVerified")] UserRegistration user)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                SendVerificationLinkEmail(user.EmailId.ToString(), system.Value, system1.Value);
        ...
        [NonAction]
        public void SendVerificationLinkEmail(string emailID, string support, string password)
        {
            User user = dbObj.Users.Where(x => x.IsEmailVerified == false && x.EmailId == emailID).FirstOrDefault();
            ...
            string subject = "NotesMarketPlace -Email Verification";
        ...
        public ActionResult Forgotpassword(Forgotpwd f)
        {
            if (ModelState.IsValid)
            {
                ...
                if (isvalid)
                {
                    ...
                    SendVerificationCode(f.EmailId.ToString(), strotp, system.Value, system1.Value);
                    dbObj.SaveChanges();
        ...
        [NonAction]
        public void SendVerificationCode(string emailId, string otp, string support, string password)
        {

            var verifyUrl = "Account/Login/?emailID=" + emailId + otp;
            var link = Request.Url.AbsoluteUri.Replace(Request.Url.PathAndQuery, verifyUrl);
            var fromEmail = new MailAddress(support, "Notes Marketplace"); //need system email
            var toEmail = new MailAddress(emailId);
            var fromEmailPassword = password; // Replace with actual password
